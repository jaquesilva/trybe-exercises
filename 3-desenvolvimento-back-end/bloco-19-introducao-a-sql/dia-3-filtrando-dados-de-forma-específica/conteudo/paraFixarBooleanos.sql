...
-- substituição ( replacement_cost ), dos 100 filmes com o maior custo de substituição, do valor 
-- mais alto ao mais baixo, entre os filmes feitos para menores de idade e que têm o custo mínimo 
...
-- 5. Mostre todos os detalhes dos clientes que não estão ativos na loja 1 . As informações podem 
-- ser encontradas na tabela customer
SELECT * FROM sakila.customer
WHERE active = 0 AND store_id = 1;

-- 6. Precisamos descobrir quais são os 50 filmes feitos apenas para adultos com a menor taxa de 
-- aluguel, para que possamos fazer uma divulgação melhor desses filmes. Em caso de empate, ordene em 
-- ordem alfabética pelo título. As informações podem ser encontradas na tabela film

-- Minha tentativa:
SELECT * FROM sakila.film
WHERE rating = 'NC-17' AND rental_rate <= 0.99
ORDER BY title ASC
LIMIT 50;
