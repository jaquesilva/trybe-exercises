    ...
    const connection = mysql.createPool({ // O método createPool cria uma pool de conexões com o banco de dados. Isso significa que a própria biblioteca irá gerenciar as múltiplas conexões que fizermos com o banco. O createPool recebe um objeto com as credenciais necessárias para estabelecer a conexão. Entre as opções possíveis, estão:

        host: 'localhost', // local onde o servidor do MySQL está armazenado. Como estamos executando localmente, usamos localhost
        user: 'trybe', // usuário que vamos utilizar para acessar o banco
        password: 'trybe2021', // senha do usuário especificado
        database: 'model_example' }); // nome do banco com o qual queremos nos conectar
