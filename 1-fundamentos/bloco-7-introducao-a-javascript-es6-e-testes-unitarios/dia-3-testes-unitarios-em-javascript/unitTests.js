...
// 1º Ideia - criar um teste

const assert = require('assert');

assert.strictEqual(typeof(summationOf), 'function', 'O resultado esperado era function') // Esse teste dará erro porque a função 'summationOf' ainda não foi criada.

// 2º Ideia - Criamos a função:

const assert = require('assert'); // Semântica

const summationOf = () => {}; // função criada

assert.strictEqual(typeof(summationOf), 'function') // Parâmetros do teste
// Esse teste não dará erro, porque criamos a função.
